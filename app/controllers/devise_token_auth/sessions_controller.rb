...
module DeviseTokenAuth
  class SessionsController < DeviseTokenAuth::ApplicationController
    ...
    def create
      ...
      if @resource && valid_params?(field, q_value) && (!@resource.respond_to?(:active_for_authentication?) || @resource.active_for_authentication?)
        ...
        if @resource.status == false
          render json: { errors: ["Your account has been deactivated temporarily. Please contact your trainer to get access to you account."]}, status: :unprocessable_entity
        else
    ...
    def render_create_error_not_confirmed
      render_error(401, I18n.t('devise_token_auth.sessions.not_confirmed', resource_email: @resource.email))
    end
